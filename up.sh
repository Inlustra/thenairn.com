...
#
#     cd /mnt/user/HQ/thenairn.com && ./up.sh
#
...
# compose` command in this directory failed on a missing file. A hand-maintained
# list of the files in a directory, kept somewhere other than that directory, is
# the bug. This script derives the list from the directory itself and prints it
# every run.
#
#
# ORDER
...
#   `./up.sh --skip control` from inside the container, or run it from the host.
#
set -euo pipefail
...
# The file list, derived from this directory. Anything with `bak` in the name is
# excluded: backup-by-copy is the house style here and there are eight of them
# sitting next to the live files.
compose_files() {
  local f
  # docker-compose.yml first (convention: base file leads the merge), rest sorted.
...
build_file_args() {
  ...
  [[ ${#FILE_ARGS[@]} -gt 0 ]] || die "no docker-compose*.yml found in $PWD"
}
...
cmd_preflight() {
  ...
  [[ -f .env ]] || die ".env is missing — run tools/bootstrap-env.sh first (it rebuilds it from 1Password)"
...
cmd_up() {
  ...
  cat <<'EOF'
   ./up.sh status                      every service and its state
...
up.sh — the single bring-up for thenairn.com (43 services, 12 compose files)
  ...
  ./up.sh                     bring the whole stack up, in stages, idempotent
  ./up.sh --build             same, but build images first (cold host)
  ./up.sh --skip control      same, but leave a stage alone (repeatable).
                              Use --skip control when running from INSIDE orca.
  ./up.sh check               validate the merged project + the stage map
  ./up.sh status              what is actually running
  ./up.sh files               the compose file list this script derives
  ./up.sh down                stop and remove the whole stack (asks first)

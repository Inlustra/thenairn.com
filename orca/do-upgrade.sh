...
log "recreating orca onto thenairncom-orca:latest (1.4.60)"
...
for i in $(seq 1 30); do
  ...
  if [ "$state" = "running" ] && [ "${restarts:-0}" -lt 2 ]; then
    ...
    ok=$((ok+1)); [ "$ok" -ge 2 ] && { log "orca is up on 1.4.60"; exit 0; }
  else
    ok=0
  fi
done

log "orca did NOT stabilise — auto-rolling back to 1.4.42-rc.5"
...
docker compose -p thenairncom -f "$COMPOSE" up -d --force-recreate orca
log "rolled back to 1.4.42-rc.5"
exit 1
